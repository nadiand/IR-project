...
def run_tests(finetune_dataset, index, test_datasets_indexes):
  ...
  model = finetune(train_corpus, train_queries, train_qrels, pyserini_qrels, nr_queries=1000, num_epochs = 5, evaluation_steps = 100)
...
#### EXTRA EXPERIMENT ####
# To confirm how good/bad our ranking performs, we compared its performance to a random reranking of the top 100 documents as retreived by BM25
# The following code allows for random reranking and evaluation of the results. This is not part of the actual experiments, but was used as a 
